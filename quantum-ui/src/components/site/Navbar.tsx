...
export function Navbar() {
  ...
  return (
    <>
      <header
        className={`sticky top-0 z-40 w-full transition-all duration-500 ${scrolled ? "py-2" : "py-4"}`}
      >
        <div className="mx-auto max-w-7xl px-4">
          <nav
            className={`glass-strong relative flex items-center justify-between rounded-2xl px-4 py-3 transition-all ${scrolled ? "shadow-[0_8px_40px_-12px_rgba(124,58,237,0.4)]" : ""}`}
          >
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="hidden h-10 w-10 place-items-center rounded-xl glass transition hover:bg-white/10 sm:grid"
              >
                <Search className="h-4 w-4" />
              </button>
              <Link
                to="/wishlist"
                className="relative hidden h-10 w-10 place-items-center rounded-xl glass transition hover:bg-white/10 sm:grid"
              >
              ...
              {user ? (
                  ...
                  <AnimatePresence>
                    {userMenu && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 8 }}
                        className="absolute right-0 top-full mt-2 w-56 rounded-2xl glass-strong p-2"
                      >
                        <Link
                          to="/dashboard"
                ...
                <Link
                  to="/login"
                  className="hidden items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-4 py-2 text-sm font-semibold text-white shadow-[0_0_20px_rgba(168,85,247,0.4)] transition hover:shadow-[0_0_30px_rgba(168,85,247,0.7)] md:inline-flex"
                >
              ...
              >
                <div className="flex flex-col gap-1">
                  ...
                  <button
                    ...
                    className="rounded-lg px-4 py-3 text-left text-sm font-medium text-white/80 hover:bg-white/10"
                  >
                    Search
                  </button>
                  ...
                  {user ? (
                    <Link
                      to="/dashboard"
                      onClick={() => setOpen(false)}
                      className="mt-3 inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-4 py-3 text-sm font-semibold"
                    >
                      <LayoutDashboard className="h-4 w-4" /> Dashboard
                    </Link>
                  ) : (
