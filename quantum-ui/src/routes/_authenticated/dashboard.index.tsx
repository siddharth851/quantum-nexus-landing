...
function DashboardPage() {
  ...
  const { data: purchases = [] } = useQuery<PurchaseWithProduct[]>({
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {stats.map((s, i) => (
          ...
          >
            <div
              className={`grid h-10 w-10 place-items-center rounded-xl bg-gradient-to-br ${s.color}`}
            >
        ...
        <div className="mt-4">
          {purchases.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-white/10 p-10 text-center">
              ...
              <Link
                to="/products"
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-4 py-2 text-sm font-semibold"
              >
