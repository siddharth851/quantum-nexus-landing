  ...
  return (
    <div className="space-y-6">
      <div className="glass-strong rounded-3xl p-8">
        ...
        <h1 className="mt-2 text-4xl font-bold"><span className="text-gradient">Settings</span></h1>
      ...
      <section className="glass-strong rounded-3xl p-6">
        ...
        <div className="mt-4">
          <Label>Bio</Label>
          <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={3} className="w-full rounded-xl glass border border-white/10 bg-white/5 p-3 text-sm outline-none focus:border-primary/60" />
        </div>
        ...
        <button onClick={saveProfile} disabled={loading} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-5 py-2.5 text-sm font-semibold disabled:opacity-60">
      ...
      <section className="glass-strong rounded-3xl p-6">
        ...
        <div className="mt-4 max-w-md">
          ...
          <button onClick={changePw} disabled={pwLoading} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-primary to-accent px-5 py-2.5 text-sm font-semibold disabled:opacity-60">
            {pwLoading ? <Loader2 className="h-4 w-4 animate-spin" /> : <KeyRound className="h-4 w-4" />} Update password
          </button>
        </div>
      </section>
    </div>
...
function Label({ children }: { children: React.ReactNode }) {
  return <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-white/50">{children}</label>;
}
function Input({ value, onChange, placeholder, type = "text", disabled }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string; disabled?: boolean }) {
  return <input type={type} value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="w-full rounded-xl glass border border-white/10 bg-white/5 px-3 py-2.5 text-sm outline-none focus:border-primary/60 disabled:opacity-50" />;
}
function Toggle({ label, checked, onChange }: { label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <label className="flex cursor-pointer items-center justify-between rounded-xl glass border border-white/10 px-4 py-3 text-sm">
      <span>{label}</span>
      <button type="button" onClick={() => onChange(!checked)} className={`relative h-6 w-11 rounded-full transition ${checked ? "bg-gradient-to-r from-primary to-accent" : "bg-white/10"}`}>
